...
public class ChatStructuredOutputMain {
    public static void main(String[] args) {
        ...
        // Crear extractor con AiServices
        ...
        Persona p;
        try {
            p = extractor.extraerPersona("Soy María, tengo 30 años y vivo en Quito");
        } catch (dev.langchain4j.exception.HttpException he) {
            System.err.println("HttpException en extractor: " + he.getMessage());
            if (GroqUtils.isModelDecommissioned(he) && config.fallbackModels() != null) {
                ...
                for (String fallback : config.fallbackModels()) {
                    try {
                        ...
                        extractor = AiServices.builder(Extractor.class).chatModel(newModel).build();
                        found = extractor.extraerPersona("Soy María, tengo 30 años y vivo en Quito");
                        break;
                    } catch (Exception ex) {
                        System.err.println("Fallback " + fallback + " falló: " + ex.getMessage());
                    }
                }
                if (found == null) throw new RuntimeException("No se pudo extraer con fallbacks");
                p = found;
            } else {
                throw he;
            }
